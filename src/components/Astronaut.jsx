...
export function Astronaut(props) {
  ...
  // Responsive position based on screen width
  const responsivePosition = useMemo(() => {
    if (viewport.width < 6) return [0.1, -1.6, 0];   // small screen (mobile)
    if (viewport.width < 10) return [1.5, -1.5, 0];    // medium screen (tablet)
    return [1.5, -1, 0];                             // large screen (desktop)
  }, [viewport.width]);

  const responsiveScale = useMemo(() => {
  if (viewport.width < 6) return 0.18; // mobile
  if (viewport.width < 10) return 0.20; // tablet
  return 0.206;                          // desktop
}, [viewport.width]);
...
// console.log("Astronaut component mounted");